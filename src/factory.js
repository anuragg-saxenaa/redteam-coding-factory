...
class CodingFactory {
  constructor(config = {}) {
    ...
    this.agentIntegration = new AgentIntegration(this, 'eng', {
      defaultTimeoutMs: config.agentTimeoutMs ?? 15 * 60 * 1000,
      pollIntervalMs: config.agentPollIntervalMs ?? 10000,
      resultsDir: path.join(this.dataDir, 'agent-results'),
    });
